...
ret_t awtk_http_on_result(http_request_t* request, int status_code, const char* status_text,
                          const char* content) {
...
ret_t http_request(http_request_t* request) {
  ...
  ret_t ret = EM_ASM_INT({ return Http.request($0, $1, $2, $3, $4, $5); }, request, request->method,
                         request->url, headers, request->body, request->body_size);
